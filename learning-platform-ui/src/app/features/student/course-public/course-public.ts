...
export class CoursePublicComponent implements OnDestroy {
  ...
  // Snapshot of loaded course — used by enroll() to know the academy slug
  ...
  subtitle(c: CourseVm)          { return (c.shortDescription || '').trim() || 'No short description yet.'; }
  categoryLabel(c: CoursePublic) { return (c.category || '').trim() || 'General'; }
  ...
  // ── Academy membership ────────────────────────────────────
  // Reads the `academyId` claim out of the stored JWT without an extra HTTP call.
  ...
  ctaLabel(c: CourseVm): string {
    ...
    if (!c.isFree)                      return 'Enroll (Payment coming soon)';
    return 'Enroll now — Free';
  }

  // Tells the template whether to render the "register" arrow icon vs "enroll" icon.
  ...
  enroll(courseId: string) {
    ...
    // Not logged in → branded login page
    if (!this.isLoggedIn) {
    ...
    // Logged in but not registered with this academy → register
    if (vm && !this.isRegisteredInAcademy(vm)) {
      sessionStorage.setItem('return_url', `/course/${courseId}`);
      this.router.navigateByUrl(slug ? `/register-student?academy=${slug}` : '/register');
      return;
    }

    // Paid course — not yet supported
    if (vm && !vm.isFree) {
      this.toast.error('Paid enrollment is not available yet.');
      return;
    }
  ...
  private toVm(res: CoursePublic): CourseVm {
    ...
    return { ...(res as any), modulesCount: modules.length, lessonsCount, previewLessonsCount, safeFullDescription: safe };
  }
  ...
  private injectFontFace(family: string, url: string) {
    ...
    const format = lower.endsWith('.woff2') ? 'woff2' : lower.endsWith('.woff') ? 'woff' : lower.endsWith('.otf') ? 'opentype' : 'truetype';
  ...
  private toReviewsVm(res: ReviewListResponse, mine: ReviewItem | null): ReviewsVm {
    ...
    return {
      summary: { avgRating: Number(res?.summary?.avgRating ?? 0), count: Number(res?.summary?.count ?? total ?? 0) },
  ...
  private prefillMyReview(mine: ReviewItem | null) {
    if (!mine) return;
    if (!(this.commentCtrl.value || '').trim()) this.commentCtrl.setValue((mine.comment || '').trim());
  ...
  reviewsPrev(vm: ReviewsVm) { if (this.reviewsPage <= 1) return; this.reviewsPage--; this.loadReviews(); }
  reviewsNext(vm: ReviewsVm) { if (this.reviewsPage >= vm.totalPages) return; this.reviewsPage++; this.loadReviews(); }
  ...
  initial(r: ReviewItem): string { return (this.displayName(r) || 'S').slice(0, 1).toUpperCase(); }
  ...
  submitReview() {
    ...
    if (!Number.isFinite(rating) || rating < 1 || rating > 5) { this.toast.error('Rating must be between 1 and 5.'); return; }
    const payload: UpsertReviewPayload = { rating, comment: comment.length ? comment : null };
    this.reviewSubmitting = true;
    this.student.upsertCourseReview(this.id, payload).subscribe({
      next: () => { this.reviewSubmitting = false; this.toast.success('Review saved.'); this.loadReviews(); },
      error: (err) => { this.reviewSubmitting = false; this.toast.error(typeof err?.error === 'string' ? err.error : 'Failed to save review.'); },
    });
  }
}