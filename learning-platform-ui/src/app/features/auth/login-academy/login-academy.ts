...
export class LoginAcademyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (!this.academySlug) {
      this.step = 'invalid';
      this.invalidError = 'No academy specified. Please open the student sign-in link from an academy page.';
      return;
    }
  ...
  forgotPassword() {
    this.router.navigate(['/forgot-password'], {
      queryParams: { academy: this.academySlug, mode: 'student' }
    });
  }
  ...
  login() {
    ...
    this.auth.loginStudent(email!, password!, this.academySlug)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe({
        next: () => {
          if (!this.auth.hasRole('Student')) {
            this.auth.logout();
            this.error = 'This account is not a student account.';
            return;
          }
          ...
          if (!tokenAcademyId || !pageAcademyId || tokenAcademyId !== pageAcademyId) {
            this.auth.logout();
            this.error = 'This student account is not linked to this academy.';
            return;
          }
